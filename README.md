...
## Git Alias
Follow these steps to create a `git quill` alias that uses Quill to generate
the commit message and then opens the editor so you can review it:
1. Create the alias globally:
   ```bash
   git config --global alias.quill '!git commit -m "$(quill)" && git commit --amend'
   ```
2. With the alias configured, use it to commit:
   ```bash
   git quill
   ```
   The command generates the message with Quill, runs the commit, and then
   opens the Git editor so you can modify it before saving.
